...
if pre_eeg_true:
    if model_name == 'ResNet':
        ...
        model = DenseNet(num_classes=500)
        model.load_state_dict(torch.load(Path), strict=False)
        model.train()
        model = model.cuda()
...
if pre_eeg_true:
    writer = SummaryWriter('./tensorboard/fewshot/{}_{}_nca_true_{}/scalar'.format(dataset, model_name, str(datetime.datetime.now().date())))
else:
    writer = SummaryWriter('./tensorboard/fewshot/{}_{}_nca_false_{}/scalar'.format(dataset, model_name, str(datetime.datetime.now().date())))

if pre_eeg_true:
    param_groups = []
    param_groups.append({'params': list(set(model.parameters()).difference(set(model.fc.parameters()))), 'lr': 1e-4})
    param_groups.append({'params': model.fc.parameters(), 'lr': 3e-4})
    param_groups.append({'params': Loss.parameters(), 'lr':3e-4})
    ...
    for batch_id in range(epoch):
        ...
        if (batch_id + 1) % 500 == 0:
            ...
            if test_correct_1 > correct_best and batch_id > 2000:
                ...
                model_name_ = 'ProxyNCA_few_shot_cosine_t_{}_{}_batch_pre_train'.format(model_name,dataset) + str(
