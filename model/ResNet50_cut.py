...
class ResNet(nn.Module):
    def __init__(self, cutParameter, block, blocks_num, num_classes=1000, include_top=True):
    ...
    def forward(self, x):
        self.conv1 = self.cutWeight(self.conv1, 0)
        x = self.conv1(x)  # conv1
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)  # maxpool
        self.layer1 = self.cutWeight(self.layer1, 1)
        x = self.layer1(x)  # conv2_x
        self.layer2 = self.cutWeight(self.layer2, 2)
        x = self.layer2(x)  # conv3_x
        self.layer3 = self.cutWeight(self.layer3, 3)
        x = self.layer3(x)  # conv4_x
        self.layer4 = self.cutWeight(self.layer4, 4)
...
def ResNet50Cut(cutParameter, num_classes=1000, include_top=True):
    return ResNet(cutParameter, Bottleneck, [3, 4, 6, 3], num_classes=num_classes, include_top=include_top)
