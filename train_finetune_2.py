...
from model.Inception_cut import Inception3Cut
from model.ResNet50_cut import ResNet50Cut
...
if pre_eeg_true:
    if model_name == 'ResNet':
        modelPre = ResNet50(num_classes=1000)
        modelPre = modelPre.cuda()
        modelPre.load_state_dict(torch.load(Path), strict=False)
        result_model = {}
        for param_tensor in modelPre.state_dict():
            if param_tensor != "fc.weight" and param_tensor != "fc.bias":
                result_model[param_tensor] = modelPre.state_dict()[param_tensor]
        model_cut = Parameter(torch.randn(1, 5)).cuda()
        model = ResNet50Cut(model_cut, num_classes=1000)
        for param_tensor in model.state_dict():
            if param_tensor in result_model.keys():
                model.state_dict()[param_tensor] = modelPre.state_dict()[param_tensor]
        ...
        modelPre = Inception3(num_classes=1000)
        modelPre = modelPre.cuda()
        modelPre.load_state_dict(torch.load(Path), strict=False)
        result_model = {}
        for param_tensor in modelPre.state_dict():
            if param_tensor != "fc.weight" and param_tensor != "fc.bias":
                result_model[param_tensor] = modelPre.state_dict()[param_tensor]
        model_cut = Parameter(torch.randn(1, 16)).cuda()
        model = Inception3Cut(model_cut, num_classes=1000)
        for param_tensor in model.state_dict():
            if param_tensor in result_model.keys():
                model.state_dict()[param_tensor] = modelPre.state_dict()[param_tensor]
        model.load_state_dict(torch.load(Path), strict=False)
...
if pre_eeg_true:
    writer = SummaryWriter('./tensorboard/fewshot/finetune2/{}_{}_nca_true_{}/scalar'.format(dataset, model_name, str(datetime.datetime.now().date())))
else:
    ...
    for batch_id in range(epoch):
        ...
        if (batch_id + 1) % 500 == 0:
            ...
            if test_correct_1 > correct_best and batch_id > 2000:
                ...
                model_name_ = 'ProxyNCA_few_shot_cosine_t_{}_{}_batch_pre_train_cut'.format(model_name,dataset) + str(
                    batch_id + 1) + '_correct_' + str(test_correct_1) + '.pth'
                torch.save(model.state_dict(), os.path.join(snapshot_dir, model_name_))
                with open(snapshot_dir + 'cutparameter/cutparameter_{}_{}_{}.txt'.format(str(datetime.datetime.now()), model_name, dataset), "w", encoding="utf-8") as f:
                    f.write(str(model.cutParameter))
